...
Explanation:
1) In this 'customer_lifecycle' table, each customer should have only 1 row of data as customer id is set as a primary key. 

2) For each group of metrics asked in the question, I saved them in seperate cte tables and left joined each cte to a distinct list of users obtained 
from the central 'rental' table after the INSERT statement, for ease of logic tracking.

3) I noticed that there are 71 customers (i.e. customer 61, customer191) whose first 30 days rentals' payment cannot be referenced from the 'payment' table 
by joining on rental ids. I set these customers' first 30 day revenue to zero.

4) Another thing I noticed with the rental date is that there are rental records from May-August 2005, then it jumps straight to Feb 2006. This should be 
investigated further in a real life setting, as it could suggest missing data.

5a) As for the interesting dimension, I decided to look at the change in revenue generated by each customer between the 30th day to the latest rental day 
compared to the first 30 days. Customers with higher revenue generated per day outside of the first 30 days (positive % increase) could mean they are 
more likely to be loyal/long-term customers. Customers with negative % increase on revenue generated could mean they are less willing to spend money in the 
long run and the company should think of ways to retain these customers, perhaps via targeted movie recommendations. 

5b) For example, customer 29 spent $21.93 on rentals in the first 30 days, which gives a revenue of $0.731/day. Between the 30th day and the 
last rental day (Feb 14 2006), customer 29 spent $116.72 ($138.65 - $21.93), giving a revenue of $0.498/day and a 31.87% drop in revenue generated 
